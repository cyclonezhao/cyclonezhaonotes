...
**线程的sleep时间不一定等于指定的毫秒数**。除了受限于底层操作系统提供的设施，更主要的是sleep可以被中断，sleep()方法本来就会抛出**中断异常**(InterruptedException)。 [ref](https://docs.oracle.com/javase/tutorial/essential/concurrency/sleep.html)
...
主码=主键=主关键字，关键字=候选码，候选关键字=候选码中除去主码的其它候选码。
...
例如地址列，值是广东省深圳市南山区xxx路xx号。假如对“广东省”和“深圳市”需要分开处理，例如更新、读取，则该列就是可分割的，不符合第一范式。

通过新增列：省份、城市，使之符合第一范式。

---

**第二范式：一张表只做一件事。**

其实定义是，非主属性不可对主属性存在部分函数依赖。

比如有一个表（学号、姓名、课程、学分、成绩）。在该表里，属性组合（学号、课程）可以标识出唯一的记录，但是“姓名”只依赖于“学号”，“学分”只依赖于“课程”，这就是所谓的部分依赖，即只依赖于主键的一部分。

该表不符合第二范式，它会带来以下问题：

1. 数据冗余：同一门课程被n个学生选修，则“学分”就重复n-1次；同一个学生选修了m门课程，“姓名”就重复了m-1次。
2. 更新异常：若需要更新某门课程的学分，则所有为该课程的行都要更新。
3. 插入异常：如果要开一门新课程，暂时没有人选修，由于没有“学号”信息，该课程也无法录入数据库。
4. 删除异常：假如一批学生已经完成了课程的选修，这些记录就应该从表中删除。但是，与此同时，“课程”和“学分”也被删除了，这可能会引发插入异常。
...
“选课表”作为中间表，关联了学生表和课程表，很好地解决了上面的问题。

---

**第三范式：保证每列（其实定义只说是非主属性）都和主键直接相关。**

其实这个和第二范式有点类似，举一个有点牵强的例子：

有一张表：（学号、姓名、性别编号、性别名称）。

姓名和性别编号都是直接依赖于主键“学号”的，但是“性别名称”却直接依赖于“性别编号”，因此只是间接依赖于主键“学号”，不符合第三范式。它带来的问题和解决方法和第二范式的描述是类似的。

---

**BC范式：保证每列（包括主属性和非主属性）都和主键直接相关。**

考虑表（仓库名、管理员、物品名、数量）。

其中，一个仓库只有一个管理员，一个管理员也只在一个仓库工作；一个仓库可存放多种物品，一种物品也可存放于多个仓库中。

这里，码是（仓库名、物品名）或（管理员、物品名）。

主属性是仓库名、管理员、物品名，非主属性是数量。

由于不存在非主属性对码的部分函数依赖，所以它符合3NF。

但是，由于主属性“管理员”，对于码（仓库名、物品名）存在部分函数依赖，所以它不符合BCNF。
...
用户通过它可实现对数据库的基本操作，例如插、删、改、排（序）、检（索）。
...
假设有一个复合索引，索引字段有F1，F2两个。从全表的角度看，F1字段的值是有序的，F2字段的值是无序的。但对于每一个指定的F1值，F2的值也是有序的。
...
1. 如果使用 F1=a 的条件查询，那么可以用二分法查询索引，索引可被高效使用
1. 如果使用 F2=7 的条件查询，那么只能线性扫描索引表，索引的使用效率低下
1. 如果使用 F1=a AND F2=7 的条件查询，那么也可以用二分法查询索引，索引可被高效使用

Q: 执行计划如何阅读？

[粗略参考](https://www.cnblogs.com/liujingyuan789/p/6061188.html)

下面是一个执行计划输出样例，我们来考察一下它的列的含义。

![执行计划](image/软件开发/执行计划.png)
...
1. const：通过索引一次就找到，只匹配一行数据
1. eq_ref: 唯一性索引扫描，对于每个索引键，表中只有一条记录与之匹配
1. ref：非唯一性索引扫描，返回匹配某个单独值的所有行，用于=、<、> 操作符带索引的列
1. range：只检索给定范围的行，使用一个索引来选择行，一般使用 between、<、>
1. index：只遍历索引树
1. ALL：全表扫描，性能最差
1. null

[参考](https://blog.csdn.net/u011431326/article/details/104119611)

**possible_keys 和 key**

前者表示查询可能使用哪些索引，后者表示实际使用了哪些索引。

> possible_keys 列中的值并不是越多越好，可能使用的索引越多，查询优化器计算查询成本时就得花费更长时间，所以如果可以的话，尽量删除那些用不到的索引。

**key_len**

表示使用的索引长度，key_len可以衡量索引的好坏，key_len越小 索引效果越好.

[参考](https://blog.csdn.net/zhangchaoyang/article/details/109501696)
...
如果一个范围（例如类、方法）内有D1，D2两个原则，那么修改D1可能会导致D2故障。
...
继承的风险：如果一个类被其他的类所继承，则当这个类需要修改时，必须考虑到所有的子类，并且父类修改后，所有涉及到子类的功能都有可能产生故障。
...
依赖倒置原则基于这样一个事实：相对于细节的多变性，抽象的东西要稳定的多。以抽象为基础搭建的架构比以细节为基础的架构要稳定的多。
...
Q: 分布式事务的一般特征是什么？主要有哪些场景？

xxx

Q: 在XA规范中，RM，TM，AP，CRM分别是啥？职责是啥？

xxx

Q: XA规范将分布式事务过程分为两个阶段，这两个阶段分别做什么？

xxx

Q: 为什么两阶段提交协议不适用于微服务场景下的跨服务分布式事务模式？
...
TCC即为try - confirm - cancel的缩写。它分为两个阶段：

1. 执行try，业务层预留资源，可能是做些预备数据。
2. 执行confirm，提交预留资源（即写入数据库）；或cancel，取消提交业务资源（即不写数据库）

在模型上，区分为一个分布式事务的发起者，和若干个分布式事务的参与者。

1. 发起者在业务逻辑代码里调用各个参与者的try方法
2. 然后在单机事务内，调用各个参与者的confirm方法，若发生异常，则在回滚代码里调用各参与者的cancel方法

在各个阶段内，发起者和参与者都需要记录日志，作为回滚的依据。

发起者记录全局事务和每个分支事务的状态和信息，参与者则记录分支事务的状态。

![TCC模型](https://yqintl.alicdn.com/5f1046f4b10628e465c8236404ac9fc26e1707ab.png)

[参考](https://www.alibabacloud.com/blog/an-in-depth-analysis-of-distributed-transaction-solutions_597232)

Q: 举一个简单例子说明TCC。

以电子商务系统为例。小明在淘宝上花了100元买了一本书，这次购买获得了10个积分。采购涉及系统中的以下操作：
...
如果在这种情况下使用 TCC 模型，则各个系统的代码必须实现以下步骤：

1）订单系统

1. Try：系统创建一个状态为“待付款”的订单。
1. confirm：系统将订单状态更新为“已完成”。
1. cancel：系统将订单状态更新为“取消”。

2) 支付系统

1. Try：假设小明账户里有1000元，购买时系统冻结100元。此时，小明账户里还有1000元的余额。
1. confirm：系统将账户余额更改为900元，并清除冻结记录。
1. cancel：系统清除冻结记录。

3）库存系统

1. Try：假设库存中有 10 本书，系统冻结其中一本。在这一步中，库存中还有 10 本书。
1. confirm：系统将库存更新为9册，并清除冻结记录。
1. cancel：系统清除冻结记录。

4）会员系统

1. Try：假设小明账户有3000积分，系统现在奖励他10积分。这一步，小明账户里的积分还是3000。
1. confirm：系统将积分更新为3010，并清除积分准备记录。
1. cancel：系统清除积分准备记录。
...
此时事务进入非原子且不一致的状态，需要根据主事务和分支事务的日志来提交或回滚剩余的分支事务，以达到最终的一致性和原子性。

Q: 按TCC进行编程时需要注意哪些点？

1. 网络异常可能会打乱两个阶段的执行顺序，使得参与者的try请求晚于cancel请求到达。
1. 因此需要支持空回滚：没有执行try的时候执行了cancel，执行了空回滚后，需要阻止try的执行。
1. 保持幂等性：当第二阶段出现网络超时等异常时，会重复调用confirm和cancel方法。因此，这两种方法的实现必须是幂等的。

Q: TCC对业务编程的优缺点是啥？

优点：TCC在业务层而不是资源层实现分布式事务，允许业务灵活选择资源锁定粒度。另外，全局事务执行过程中不会一直持有锁，因此系统吞吐量远高于两阶段提交XA模式。

缺点：TCC事务模型对业务有侵入性，业务方需要将一个接口拆分为三个，开发成本高。

# git

Q: 回退代码

1. 回退本地代码库：git reset --hard ID
1. 推送到远程服务器：git push -f -u origin 分支名
1. 重新拉代码：git pull
