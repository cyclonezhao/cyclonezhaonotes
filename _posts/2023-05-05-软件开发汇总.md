...
当MQ发消息给消费者1，消费者1消费完毕，却在最后ack的时候失败了。为了避免消息丢失，保证消息最少会被消费一次，此时MQ会重发同样的消息给消费者2。

这时候发生了消息重复的问题，如果消费者的业务逻辑本身不天然幂等，就得加限制避免重复消费。

**方案一：**

在数据库加一张任务表（id，消费状态），消息内容是任务id。消费者接到消息后：

1. 首先按id查任务状态，如果不是”未消费“，则终止后续逻辑执行；
2. 否则立刻将状态更新为”消费中“。

如果是高并发场景，方案一有两个缺点：

1. 多个消费者同时查询到”未消费“的状态；
2. 多个消费者同时查库，拖垮数据库。
...
3. a，b的读取和更新是原子的，比如是同一张表的两个字段。如果不是同一张表，就需要用事务实现原子性。

若满足以上特性，则这个业务逻辑自身是天然幂等的。

比如固定资产折旧场景：

1. 累计折旧，本期折旧额，同时作为计提折旧的输入；
2. 计算过程：

	1. 先将累计折旧 减去 本期折旧额，得到期初累计折旧；
	2. 然后用期初累计折旧，根据具体的折旧方法，计算出新的本期折旧额
	3. 执行赋值：本期折旧额=新的本期折旧额，累计折旧=期初累计折旧+新的本期折旧额

3. 由于累计折旧，本期折旧额是同一张表的两个字段，所以读取和更新是原子的。

**方案三**，针对方案1的改进：

1. 这张任务表要搭配缓存使用；
2. 消费者首先查缓存，若查到，且状态不是”未消费“，则可立刻终止后续逻辑执行；
3. 如果查不到，有两种可能性：

	1. 这个消息还没有被消费过；
	2. 这个消息正在被别的消费者消费，但还没来得及写库更新状态，或者写库了，但缓存还没来得及更新；

4. 由于上述的第2种可能性，所以立刻加分布式锁，若不能获得锁，则立刻终止后续逻辑执行，否则就写库更新状态，并更新缓存。

**方案四**

方案三的状态只有（未消费，消费中，已消费）三种取值，且状态是层层递进的。对于非枚举类型的结果字段，则采用版本号机制避免重复消费带来影响：

1. SELECT id, result, someOtherFields, version oldVersion FROM table WHERE condition;
2. 计算新的result；
3. 加分布式锁，如果成功获得锁，就执行UPDATE table SET result=newResult, version=version+1 WHERE id=查出的id AND version=oldVersion;

**方案五**

有些场景，可以利用数据库的唯一约束，杜绝重复消息造成的影响；
...
从内心获取能量的，同时靠直觉获取信息的，这类人有一个自我的小世界，也很喜欢研究，整天自己跟自己聊天。

如果具体的人很聪明，则让他们负责研究，同时需要花适当精力帮助他们把控进度。

**一个人同时具有T，J属性**

这类人做事严格，根据成本收益，不太会被情感影响决策。自我要求严格、冷酷。

让他们负责管理、协调类工作。

**具备P属性的人，搭配具备J属性的人**

完美的配合：J做计划，P就跟着J走，不容易吵架。
